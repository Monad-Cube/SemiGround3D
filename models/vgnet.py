...
class VGNet(nn.Module):
    ...
    def __init__(self, input_feature_dim, args, data_config):
        ...
        self.num_heading_bin = data_config.num_heading_bin  # = 1
        ...
        for i in range(self.args.num_decoder_layers):
            use_ref_branch = False
            if self.args.ref_each_stage: # true
                use_ref_branch = True
            else:
            ...
            if args.size_cls_agnostic:
                ...
                self.prediction_heads.append(PredictHead(self.num_class, 
                    ...
                    use_ref_branch=use_ref_branch,              # True
                    use_cls_branch=use_cls_branch,              # True
                    use_ref_mask=self.args.use_ref_mask,        # False
                    use_objectness=self.args.use_objectness,    # True
                    args=self.args))
    ...
    def forward(self, data_dict):
        ...
        for i in range(self.args.num_decoder_layers):
            ...
            data_dict[f'{prefix}cross_object_att'] = cross_object_att
            data_dict[f'{prefix}cross_lang_att'] = cross_lang_att
            ...
            base_xyz, base_size, fused_feat_in_head = self.prediction_heads[i](object_feat_pre,
                                                        base_xyz=xyz,
                                                        end_points=data_dict,
                                                        prefix=prefix,
                                                        lang_feat=lang_feat,
                                                        ...
                                                        prefix_index=i)
            # for OtaMatch
            data_dict[f'{prefix}fused_feat_in_head'] = fused_feat_in_head
            base_xyz = base_xyz.detach().clone()
            base_size = base_size.detach().clone()
            # True, and i isn't 0, TPM point selection
            if self.args.use_att_score and i in self.args.ref_filter_steps:
                filter_scores = cross_lang_att.mean(dim=1)           # [B, K]
                select_num = int(filter_scores.shape[1] * self.args.ref_mask_scale)     # next layer point number, shape[1]*0.5
                select_index = torch.topk(filter_scores, select_num, dim=1)[1].long()   # index of selected point, topk return (values, indices)
            ...
            # transfer output feature into input feature for the next layer
            if self.args.transformer_mode == 'serial':
                ...
                input_object_feat = object_feat
                input_lang_feat = lang_feat
            if i == 4: data_dict['final_xyz'] = base_xyz
        return data_dict
