...
from lib.loss_helper import get_supervised_loss, get_consistency_loss
...
class Solver():
    def __init__(self, student, teacher, data_config, dataloader, optimizer, stamp, val_freq=1, args=None,
    ...
    def __call__(self, epoch, verbose):
        ...
        self._total_iter["semi_train"] = len(self.dataloader["burn_in"]) * epoch
        ...
        for epoch_id in range(epoch):
            try:
                ...
                # feed one epoch
                self._feed(self.dataloader["burn_in"], self.dataloader["burn_in"], "train", epoch_id)

                # save model
                self._log("saving last models...\n")
                model_root = os.path.join(CONF.PATH.OUTPUT, self.stamp)
                torch.save(self.student.state_dict(), os.path.join(model_root, "model_last.pth"))
                torch.save(self.teacher.state_dict(), os.path.join(model_root, "teacher_last.pth"))
    ...
    def _feed(self, labeled_dataloader, unlabeled_dataloader, phase, epoch_id):
        ...
        #    checkpoint = torch.load(os.path.join(CONF.PATH.OUTPUT, "10%_3D-SPS", "checkpoint.tar"))
        #    self.teacher.load_state_dict(checkpoint["model_state_dict"])
    
        # Transfer teacher's model parameters to Student
        ...
        for data_dict in unlabeled_dataloader:
            ...
            with torch.no_grad():
                start = time.time()
                t_data_dict = self._forward(data_dict, "generate_pesudo_labels")
    ...
    def _dump_log(self, phase):
        log = {
            ...
            "semi_train": {
                "loss": ["consistency_loss","ref_consistency_loss"],
            },
            'val': {
                "loss": ["loss", "ref_loss", "ref_mask_loss", "lang_cls_loss", "objectness_loss", "kps_loss", "box_loss", "sem_cls_loss"],
    ...
    def _epoch_report(self, epoch_id):
        ...
        epoch_report = self.__epoch_report_template.format(
            ...
            ref_consistency_loss=round(np.mean([v for v in self.log["semi_train"]["ref_consistency_loss"]]), 5),
            consistency_loss=round(np.mean([v for v in self.log["semi_train"]["consistency_loss"]]), 5),
