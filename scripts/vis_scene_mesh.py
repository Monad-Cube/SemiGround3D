...
def dump_results(args, scanrefer, data, config, split, scanrefer_id):
    dump_dir = os.path.join(CONF.PATH.OUTPUT, "vis_scene_mesh")
    ...
    point_clouds = data['point_clouds'].squeeze(0).cpu().numpy()
    #batch_size = point_clouds.shape[0]

    pcl_color = data["pcl_color"].squeeze(0).detach().cpu().numpy()
    ...
    mesh.write(os.path.join(scene_dump_dir, '{}mesh.ply'.format(scene_id)))
    
    # write pointcloud
    #write_ply_rgb(point_clouds, pcl_color, os.path.join(scene_dump_dir, 'strong_pc.ply'))
    #write_ply_rgb(ema_point_clouds, ema_pcl_color, os.path.join(scene_dump_dir, 'weak_pc.ply'))
...
def visualize(args):
    ...
    for scanrefer_id, data in enumerate(dataloader):
        ...
        print("visualize scene mesh done!")
        return
