...
def get_dataset(args, scanrefer, all_scene_list, split, config):
...
def dump_results(args, scanrefer, data, config, split, scanrefer_id):
    path = CONF.PATH.OUTPUT
    dump_dir = os.path.join(CONF.PATH.OUTPUT, "vis_all_bbox")
    ...
    pcl_color = data["pcl_color"]
    if args.use_color:
        pcl_color = (pcl_color * 256 + MEAN_COLOR_RGB).astype(np.int64)

    
    # ground truth
    gt_center = data['center_label'] # MAX_NUM_OBJ,3)
    gt_heading_class = data['heading_class_label'] # K2
    gt_heading_residual = data['heading_residual_label'] # K2
    gt_size_class = data['size_class_label'] # K2
    gt_size_residual = data['size_residual_label'] # K2,3
    # reference
    gt_ref_labels = data["ref_box_label"]
    ...
    write_bbox(gt_obb, 1, os.path.join(scene_dump_dir, 'gt_Blue_{}_{}.ply'.format(object_id, object_name)))

    for id in range(output_number):
        if id!=gt_ref_idx:
            all_obb = config.param2obb(gt_center[id, 0:3], gt_heading_class[id], 
                                      gt_heading_residual[id], gt_size_class[id], 
                                      gt_size_residual[id])
            write_bbox(all_obb, 1, os.path.join(scene_dump_dir, 
                                               'obj{}_{}_{}.ply'.format(id, object_id, object_name)))
...
def visualize(args):
    ...
    # dataset
    scanrefer_id = 0
    dataset = get_dataset(args, scanrefer, scene_list, split, DC)
    ...
    # visualize
    dump_results(args, scanrefer, dataset, DC, split, scanrefer_id)
