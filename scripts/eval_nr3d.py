...
def get_dataloader(args, m3drefer, all_scene_list, split, config):
...
def get_model(args, config):
    ...
    model_name = "model_last.pth" if args.detection else "teacher_epoch12.pth"
...
def get_nr3d(args):
    ...
    scene_list = sorted(list(set([data["scene_id"] for data in nr3d])))
    if args.num_scenes != -1:
        scene_list = scene_list[:args.num_scenes]

    nr3d = [data for data in nr3d if data["scene_id"] in scene_list]

    return nr3d, scene_list
...
def eval_ref(args):
    ...
    nr3d, scene_list = get_nr3d(args)

    # dataloader
    _, dataloader = get_dataloader(args, nr3d, scene_list, "val", DC)
        ...
        for seed in seeds:
            ...
            for data in tqdm(dataloader):
                ...
                for i in range(ids.shape[0]):
                    idx = ids[i]
                    scene_id = nr3d[idx]["scene_id"]
                    object_id = nr3d[idx]["object_id"]
                    ann_id = nr3d[idx]["ann_id"]
    ...
    for key, value in predictions.items():
        ...
        scene_pred[scene_id].append({
            "object_id": key[1], # for scanrefer and nr3d
            "ann_id": key[2],
            "aabb": value.tolist()
        })
    # TODO, change the path
    prediction_output_root_path = "/workspace/yf/projects/Semi-SPS/nr3dpredictions/10%_nr3d_soft" 
