...
from lib.dataset import ScannetReferenceDataset, ScannetMulti3DReferDataset
...
M3DREFER_TRAIN = json.load(open(os.path.join(CONF.PATH.DATA, "multi3drefer_train.json")))
...
def get_data_m3(args, m3drefer, all_scene_list, split, config):
    # Create Dataset and Dataloader
...
def get_m3drefer(args):
    if args.use_train:
        m3drefer = M3DREFER_TRAIN
    elif args.use_test:
        m3drefer = M3DREFER_VAL
    else:
        m3drefer = M3DREFER_VAL
    all_scene_list = sorted(list(set([data["scene_id"] for data in m3drefer])))
    if args.scene_id:
        assert args.scene_id in all_scene_list, "The scene_id is not found"
        scene_list = [args.scene_id]
    else:
...
def dump_results(args, scanrefer, data, config, split, scanrefer_id):
    dump_dir = os.path.join(CONF.PATH.OUTPUT, "vis_m3_appendix")
    ...
    write_bbox(pred_obb, 1, os.path.join(scene_dump_dir, '20%_soft_pred_{}_{}_{}_{:.5f}_{:.5f}.ply'.format(object_id, object_name, ann_id, pred_ref_scores_softmax[pred_ref_idx], iou)))
...
def visualize(args):
    ...
    m3drefer, scene_list = get_m3drefer(args)
    if args.use_train:
        split = 'train'
    elif args.use_test:
        split = 'test'
        ...
        split = 'val'
    # dataloader
    _, dataloader = get_data(args, m3drefer, scene_list, split, DC)
    ...
    # evaluate
    print("visualizing...")
    for m3drefer_id, data in enumerate(dataloader):
        if data["object_id"] == object_id and data["ann_id"] == ann_id :
            ...
            # visualize
            dump_results(args, m3drefer, data, DC, split, m3drefer_id)
            print("done!")
            return
