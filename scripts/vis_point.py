...
def dump_results(args, scanrefer, data, config, split, scanrefer_id):
    dump_dir = os.path.join(CONF.PATH.OUTPUT, "vis_point")
    ...
    #write_bbox(pred_obb, 1, os.path.join(scene_dump_dir, 'sps_pred_{}_{}_{}_{:.5f}_{:.5f}.ply'.format(object_id, object_name, ann_id, pred_ref_scores_softmax[pred_ref_idx], iou)))
    ...
    write_ply_rgb(filter_points, np.array(filter_points_rgb)[np.newaxis, :].repeat(filter_points_num, 0), os.path.join(scene_dump_dir, 'filter_{}_{}_{}_{}.ply'.format(object_id, object_name, ann_id, iou)))
