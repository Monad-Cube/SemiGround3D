...
from lib.eval_helper import get_eval_m3_train
...
class Solver():
    def __init__(self, student, teacher, training_stage, data_config, dataloader, scanrefer, optimizer_b, optimizer_s, stamp, val_freq=1, args=None,
    ...
    lr_decay_step=None, lr_decay_rate=None, bn_decay_step=None, bn_decay_rate=None, distributed_rank=None):
        ...
        self.__epoch_report_template_semi = EPOCH_REPORT_TEMPLATE_SEMI
        self.__best_report_template = BEST_REPORT_TEMPLATE

        # lr scheduler
        if lr_decay_step and lr_decay_rate:
            if isinstance(lr_decay_step, list):
                if self.training_stage == 'burn_in':
                    self.lr_scheduler = MultiStepLR(optimizer_b, lr_decay_step, lr_decay_rate)
                else:
                    self.lr_scheduler = MultiStepLR(optimizer_s, lr_decay_step, lr_decay_rate)
            else:
                if self.training_stage == 'burn_in':
                    self.lr_scheduler = StepLR(optimizer_b, lr_decay_step, lr_decay_rate)
                else:
        ...
        if bn_decay_step and bn_decay_rate:
            ...
            if self.training_stage == 'burn_in':
                self.bn_scheduler = BNMomentumScheduler(self.teacher, bn_lambda=bn_lbmd, last_epoch=-1)
            else:
    ...
    def __call__(self, epoch, verbose):
        ...
        for epoch_id in range(epoch):
            try:
                ...
                if self.training_stage == "semi":
                    torch.save(self.teacher.state_dict(), os.path.join(model_root, "teacher_epoch{}.pth".format(epoch_id+1)))    
                #if epoch_id in [18, 19, 20, 21, 22, 23, 24, 25]:
    ...
    def _burn_in(self):
        ...
        if CONF.use_10p_model:
            self._log("Loading 20% m3 trained model state_dict..\n")
            ...
            path = os.path.join(CONF.PATH.OUTPUT, "10%_nr3d", "model_best.pth")
        ...
        print("Burn-in finish!")

    def _forward(self, data_dict, stage):
        if stage == "burn_in" or stage == "generate_pesudo_labels" or stage == "burn_in_val": data_dict = self.teacher(data_dict)
        elif stage == "semi_train" or stage == "val" or stage == "semi_val": data_dict = self.student(data_dict)
        else:
    ...
    def _filter_pseudo_label(self, data_dict) -> bool:
        ...
        if(use_average): 
            ...
            if self.args.weighting_mode == "not_weighting":
                ...
                if self.args.use_jointly_filtering: # jointly filtering
                    batch_filter_mask[np.where(obj_soft_score < self.obj_thr)] = 0
            elif self.args.weighting_mode == "linear_weighting":
    ...
    def _feed(self, labeled_dataloader, unlabeled_dataloader, phase, epoch_id):
        ...
        #self.teacher.load_state_dict(checkpoint["model_state_dict"])
        if self.training_stage == "burn_in":
            self._log("Burning in stage start\n")
            phase = "burn_in"
            # switch mode
            self._set_phase(phase)
            # re-init log
        ...
        # --------------- semi-train ---------------
            ...
            for data_dict in unlabeled_dataloader:
                ...
                t_data_dict['pcl_color'] = t_data_dict['ema_pcl_color']
    ...
    def _val(self, dataloader, phase, epoch_id):
        ...
        for data_dict in dataloader:
            ...
            with torch.no_grad():
                # forward
                if self.training_stage == "burn_in":
                    data_dict = self._forward(data_dict, "burn_in_val")
                elif self.training_stage == "semi":
                    data_dict = self._forward(data_dict, "semi_val")
        ...
        if cur_best > self.best[cur_criterion]:
            ...
            model_root = os.path.join(CONF.PATH.OUTPUT, self.stamp)
            if self.training_stage == "burn_in":
                torch.save(self.teacher.state_dict(), os.path.join(model_root, "model.pth"))
            elif self.training_stage == "semi":
    ...
    def _epoch_report(self, epoch_id):
        ...
        if self.training_stage == "burn_in":
            ...
            epoch_report = self.__epoch_report_template_semi.format(
                train_loss=round(np.mean([v for v in self.log["burn_in"]["loss"]]), 5),
                train_ref_loss=round(np.mean([v for v in self.log["burn_in"]["ref_loss"]]), 5),
                train_ref_mask_loss=round(np.mean([v for v in self.log["burn_in"]["ref_mask_loss"]]), 5),
                train_lang_cls_loss=round(np.mean([v for v in self.log["burn_in"]["lang_cls_loss"]]), 5),
    ...
    def evaluate_detection_one_epoch(self, test_loader, config):
        ...
        for batch_idx, batch_data_label in enumerate(test_loader):
            ...
            with torch.no_grad():
                if self.training_stage == "burn_in":
                    data_dict = self.teacher(batch_data_label)
                elif self.training_stage == "semi":
